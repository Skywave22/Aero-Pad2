...
import com.aeropad.remote.domain.VoiceCommandParser
import com.aeropad.remote.viewmodel.VoiceInputViewModel
import com.aeropad.remote.voice.VoiceInputManager
...
fun VoiceInputSheet(
    ...
    if (showRationale) {
        AlertDialog(
            ...
            text = {
                Text(
                    "AeroPad uses the microphone ONLY while you hold a dictation session, " +
    ...
    ModalBottomSheet(onDismissRequest = { viewModel.stopListening(); onDismiss() }) {
        Column(modifier = Modifier.padding(horizontal = 20.dp)) {
            ...
            if (permissionDenied) {
                Text(
                    "Microphone permission denied — enable it in system Settings → Apps → AeroPad.",
                    style = MaterialTheme.typography.bodyMedium,
                    color = MaterialTheme.colorScheme.error,
                    modifier = Modifier.padding(top = 6.dp)
                )
