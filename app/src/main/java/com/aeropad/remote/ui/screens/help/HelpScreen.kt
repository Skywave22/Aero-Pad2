package com.aeropad.remote.ui.screens.help
...
fun HelpScreen(onBack: () -> Unit) {
    val faqs = listOf(
        "PC can't find my phone" to
            "Tap Prepare PC pairing on the Connect screen first - this makes the phone visible for 2 minutes. Keep AeroPad open while pairing.",
        "Paired but not connected" to
            "Remove the pairing on BOTH sides (Windows Bluetooth settings AND phone Bluetooth settings), then pair again from Windows while AeroPad is open.",
        ...
        "Connection drops when app is in background" to
            "AeroPad runs a foreground service to keep the link alive. Disable battery optimization for AeroPad in Android settings if drops continue.",
