package com.aeropad.remote.keyboard

import com.aeropad.remote.model.HidKeys
import com.aeropad.remote.model.keyboard.DefaultKeyboards
import com.aeropad.remote.model.keyboard.KeySpec
import com.aeropad.remote.model.keyboard.KeyboardLayoutSpec
import com.aeropad.remote.model.keyboard.ShortcutPacks
...
class FnLayerTest {
    ...
    fun `fn f9 to f12 resolve to real media usages`() {
        val board = DefaultKeyboards.fullQwerty()
        val expected = mapOf(
            "f9" to com.aeropad.remote.model.HidConsumer.PLAY_PAUSE,
            "f10" to com.aeropad.remote.model.HidConsumer.MUTE,
            "f11" to com.aeropad.remote.model.HidConsumer.VOLUME_DOWN,
            "f12" to com.aeropad.remote.model.HidConsumer.VOLUME_UP
        )
    ...
    fun `one handed mode enum round trips by name`() {
        com.aeropad.remote.data.keyboard.OneHandedMode.values().forEach { m ->
            assertEquals(
                m,
                com.aeropad.remote.data.keyboard.OneHandedMode.valueOf(m.name)
            )
        }
        // Unknown string is what the store guards against with getOrDefault.
        assertTrue(
            runCatching {
                com.aeropad.remote.data.keyboard.OneHandedMode.valueOf("SIDEWAYS")
            }.isFailure
        )
    }
