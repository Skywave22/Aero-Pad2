...
import com.aeropad.remote.domain.PropertyCatalog
...
class PropertyCatalogTest {
    ...
    fun `gamepad control count matches the real constructor`() {
        val specParams = ctorParams(com.aeropad.remote.model.gamepad.GamepadControlSpec::class)
        val pressed = ctorParams(com.aeropad.remote.model.gamepad.PressedStyle::class)
    ...
    fun `gamepad layout meta matches`() {
        val params = ctorParams(com.aeropad.remote.model.gamepad.GamepadLayoutSpec::class)
        assertEquals(PropertyCatalog.GAMEPAD_LAYOUT_PROPS, params - 1)   // − controls list
    }

    @Test
    fun `keyboard key count matches`() {
        val params = ctorParams(com.aeropad.remote.model.keyboard.KeySpec::class)
        assertEquals(PropertyCatalog.KEYBOARD_KEY_PROPS, params - 1)     // − id
    }

    @Test
    fun `widget count matches frame and style expansion`() {
        val spec = ctorParams(com.aeropad.remote.model.widgets.WidgetSpec::class)
        val frame = ctorParams(com.aeropad.remote.model.widgets.WidgetFrame::class)
        val style = ctorParams(com.aeropad.remote.model.widgets.WidgetStyle::class)
    ...
    fun `settings count matches the four settings classes`() {
        val total = ctorParams(com.aeropad.remote.model.AppSettings::class) +
            ctorParams(com.aeropad.remote.model.MouseSettings::class) +
            ctorParams(com.aeropad.remote.model.KeyboardSettings::class) +
            ctorParams(com.aeropad.remote.model.GamepadSettings::class)
        assertEquals(PropertyCatalog.SETTINGS_PROPS, total)
    }

    @Test
    fun `macro step props equal the largest real step type`() {
        val largest = com.aeropad.remote.model.macros.MacroStep::class.sealedSubclasses
    ...
    fun `keyboard instances derive from the real default board`() {
        val board = com.aeropad.remote.model.keyboard.DefaultKeyboards.fullQwerty()
        val expected = board.rows.sumOf { it.size } +
            com.aeropad.remote.model.keyboard.KeyboardLayoutSpec.FAVORITES_MAX +
            com.aeropad.remote.model.keyboard.KeyboardLayoutSpec.FN_OVERLAY_MAX
